        ...
        <main class="main py-5" style="background: #f8f5ee;">
            <div class="container">
                ...
                <div class="text-center mb-5">
                    <img src="{{ asset('assets/icons/map.svg') }}" alt="Map Icon" style="width: 40px; height: 40px;">
                    ...
                    @foreach($spots as $i => $spot)
                        <div class="col-12 col-md-6 col-lg-4">
                            <div class="card h-100 border-0 shadow-sm rounded-4 spot-card fade-in" style="cursor:pointer;"
                                data-bs-toggle="modal" data-bs-target="#spotModal" data-title="{{ $spot['title'] }}"
                                data-image="{{ asset($spot['image']) }}" data-description="{{ $spot['description'] }}">
                                <div class="overflow-hidden rounded-top-4">
                                    <img src="{{ asset($spot['image']) }}" class="card-img-top spot-img"
                                        alt="{{ $spot['title'] }}"
                                        style="object-fit: cover; height: 180px; transition: transform 0.3s;">
                                </div>
                                <div class="card-body">
                                    <h5 class="card-title fw-bold" style="color: #114f89;">{{ $spot['title'] }}</h5>
                                    <p class="card-text text-muted mb-0">{{ $spot['description'] }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
